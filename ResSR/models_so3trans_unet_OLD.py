...
# tensor product wrappper for e3nn :)
class SelfTP(nn.Module):
    def __init__(self, in_irreps, out_irreps, *, dev):
        super().__init__()
        self.tp = FullyConnectedTensorProduct(in_irreps, in_irreps, out_irreps, internal_weights=True, shared_weights=True).to(dev)
...
def equiv_core(n_dir: int, h_s=32, h_v=8, *, dev):
    ...
    gate = Gate(s_mid, [nn.GELU()], g_mid, [nn.Tanh()], v_mid).to(dev)
...
class SO3Transformer(nn.Module):
    ...
    def forward(self, sh29):                 # (..., 29)
        ...
        if (self.core is None or amp.shape[-1] != self.core[0].tp.irreps_in1.dim):
            self.core = equiv_core(amp.shape[-1], self.h_s, self.h_v, dev=dev)
...
class VoxelSO3(nn.Module):
    def __init__(self, hidden=64):
        super().__init__()
        self.core=SO3Transformer(level=2)
